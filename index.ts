...
class MysqlConnector extends AdminForthBaseConnector implements IAdminForthDataSourceConnector {
  ...
  getFilterString(filter: IAdminForthSingleFilter | IAdminForthAndOrFilter): string {
    if ((filter as IAdminForthSingleFilter).field) {
      ...
      if ((filter as IAdminForthSingleFilter).rightField) {
        ...
        const operator = this.OperatorsMap[(filter as any).operator as keyof typeof this.OperatorsMap] as string;
        return `${left} ${operator} ${right}`;
      }
      // filter is a Single filter
      let placeholder = '?';
      let field = (filter as IAdminForthSingleFilter).field;
      let operator = this.OperatorsMap[(filter as any).operator as keyof typeof this.OperatorsMap] as string;
    ...
    return (filter as IAdminForthAndOrFilter).subFilters.map((f) => {
      ...
      return `(${this.getFilterString(f)})`;
    }).join(` ${this.OperatorsMap[(filter as any).operator as keyof typeof this.OperatorsMap] as string} `);
  }
  ...
  async getAggregateWithOriginalTypes({ resource, filters, aggregations, groupBy }: {
    ...
    for (const [alias, rule] of Object.entries(aggregations)) {
      ...
      switch (rule.operation) {
        ...
        case 'median': medianFields.push({ alias, field: rule.field as string }); break;
      }
    }
  ...
  async getDataWithOriginalTypes({ resource, limit, offset, sort, filters, columns }: { resource: AdminForthResource; limit?: number; offset?: number; sort: Array<{ field: string; direction: AdminForthSortDirections }>; filters: IAdminForthAndOrFilter; columns?: Array<{ name: string }> }): Promise<any[]> {
